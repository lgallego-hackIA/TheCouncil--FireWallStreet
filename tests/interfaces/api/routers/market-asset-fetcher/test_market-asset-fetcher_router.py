...
router_path = os.path.join(PROJECT_ROOT, "src", "interfaces", "api", "routers", "market-asset-fetcher", "router.py")
...
def test_list_market_asset_fetchers():
    """Test listing all market-asset-fetchers endpoint"""
    response = client.get("")
    assert response.status_code == 200
    assert "message" in response.json()

def test_get_market_asset_fetcher():
    """Test getting a specific market-asset-fetcher by ID endpoint"""
    item_id = "test123"
    response = client.get(f"/{item_id}")
    assert response.status_code == 200
    assert response.json()["id"] == item_id

def test_create_market_asset_fetcher():
    """Test creating a new market-asset-fetcher endpoint"""
    test_data = {"name": "Test market-asset-fetcher", "value": 123}
    response = client.post("", json=test_data)
    assert response.status_code == 200
    assert response.json()["data"] == test_data

def test_update_market_asset_fetcher():
    """Test updating a market-asset-fetcher endpoint"""
    item_id = "test123"
    test_data = {"name": "Updated market-asset-fetcher", "value": 456}
    response = client.put(f"/{item_id}", json=test_data)
    assert response.status_code == 200
    assert response.json()["id"] == item_id
    assert response.json()["data"] == test_data

def test_delete_market_asset_fetcher():
    """Test deleting a market-asset-fetcher endpoint"""
    item_id = "test123"
...
def test_verify_automation_exists():
    ...
    router_dir = os.path.join(PROJECT_ROOT, "src", "interfaces", "api", "routers", "market-asset-fetcher")
    ...
    test_dir = os.path.join(PROJECT_ROOT, "tests", "interfaces", "api", "routers", "market-asset-fetcher") 
    ...
    openapi_file = os.path.join(PROJECT_ROOT, "data", "openapi", "market-asset-fetcher.json")
    ...
    blob_file = os.path.join(PROJECT_ROOT, "data", "blobs", "automations", "market-asset-fetcher.json")
    ...
    if os.path.exists(blob_file):
        with open(blob_file, "r") as f:
            ...
            assert automation_data["name"] == "market-asset-fetcher", f"Expected name market-asset-fetcher but got {automation_data.get('name')}"
